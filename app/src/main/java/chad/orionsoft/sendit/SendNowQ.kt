...
class SendNowQ : AppCompatActivity() {
    ...
    private lateinit var sendList:ArrayList<SendObjectQ>
    private lateinit var sAdapter: SendAdapterQ
    ...
    inner class SendAdapterQ(private val sList:ArrayList<SendObjectQ>) : RecyclerView.Adapter<SendAdapterQ.SendHolder>() {
    ...
    private suspend fun sendFilesOnTCPAsync(selectedItems:ArrayList<SendObjectQ>) : Deferred<String> =
        coroutineScope {
            async(Dispatchers.IO) {
                var res:String
                val sendObjects=ArrayList<SendObjectQ>()
                ...
                try {
                    ...
                    for(i in 0 until sendObjects.size) {
                        ...
                        if(responseString.contains(Connection.RECEIVER_OK)) {
                            ...
                            val stream = contentResolver.openInputStream(sendObjects[i].uri)
                            Log.d("availableStream", "${stream?.available()}")
                            var loops=0
                     ...
                     /*       while(stream!!.available()>0 && !skipFile) {
                            ...
                            while(!skipFile) {
                                val fileBuff = ByteArray(4096)
                                val bytesRead = stream?.read(fileBuff) ?: -1
